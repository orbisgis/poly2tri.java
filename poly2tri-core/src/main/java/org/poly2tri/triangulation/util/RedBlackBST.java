...
public class RedBlackBST<Key extends Comparable<Key>, Value>
{
    ...
    private final int            species;      // species kind of tree for
                                                // insert
    ...
    public Value findLowerOrEqual( double key )
    {
        ...
        if( key == ((Double)root.key).doubleValue() )
        {
            return root.value;
        }
        else if( key < ((Double)root.key).doubleValue() )
        {
            // smaller
    ...
    {
        if( key == ((Double)node.key).doubleValue() )
        {
            return node;
        }        
        else if(  key < ((Double)node.key).doubleValue() )
        {
            // smaller
            if( node.left != null )
            {
